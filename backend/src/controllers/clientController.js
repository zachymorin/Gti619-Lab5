...
export const getResidentialClients = async (req, res) => {
    ...
    db.all(query, ['residentiel'], (err, rows) => {
        if (err) {
            return res.status(500).json({ error: "Erreur lors de la récupération des clients résidentiels." });
        }
...
export const getBusinessClients = async (req, res) => {
    ...
    db.all(query, ['affaire'], (err, rows) => {
        if (err) {
            return res.status(500).json({ error: "Erreur lors de la récupération des clients d'affaires." });
        }
        return res.json(rows);
    });
};