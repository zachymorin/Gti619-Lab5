...
export const updateSecurityConfig = async (req, res) => {
    ...
    db.serialize(() => {
        ...
        db.run("INSERT INTO security_logs (event) VALUES (?)", ["Paramètres de sécurité mis à jour par l'Administrateur."]);

        if (hasError) {
            return res.status(500).json({ error: "Certains paramètres n'ont pas pu être mis à jour." });
        } else {
            return res.json({ message: "Configurations de sécurité mises à jour avec succès." });
        }
    });
};
...
export const createUser = async (req, res) => {
    ...
    db.get("SELECT value FROM security_config WHERE key = 'min_length'", (err, config) => {
        if (err) return res.status(500).json({ error: "Erreur lors de la vérification des politiques de sécurité." });

        const minLength = config ? parseInt(config.value) : 8;
        if (password.length < minLength) {
            return res.status(400).json({ error: `Le mot de passe doit contenir au moins ${minLength} caractères.` });
        }
        ...
        db.get("SELECT id FROM users WHERE username = ?", [username], (err, existingUser) => {
            ...
            db.run(insertQuery, [username, hash, salt, role], function(err) {
                ...
                db.run("INSERT INTO security_logs (event) VALUES (?)", [`Nouvel utilisateur créé par l'admin : ${username} (${role})`]);
